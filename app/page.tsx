...
const searchInputStyles =
  "absolute top-0 w-full transform -translate-x-1/2 max-w-[500px] border border-gray-200 rounded-lg bg-white/95 text-sm py-3 px-4 focus:outline-none focus:ring-1 focus:ring-gray-300 shadow-md text-black"
...
export default function Page() {
  ...
  useEffect(() => {
    const loadMenuData = async () => {
      ...
      try {
        // Load food menu JSON
        const foodMenuResponse = await fetch("/food-menu.json")
        const foodMenuData = await foodMenuResponse.json()
        setMenuItems(foodMenuData)

        // Load bar menu JSON
        const barMenuResponse = await fetch("/bar-menu.json")
    ...
    <main className={`h-[100dvh] flex flex-col ${playfair.variable} font-sans overflow-hidden`}>
      ...
      <header className="flex flex-col bg-[#8B0000] text-white">
        ...
        <div className="flex justify-center items-center py-2">
          <div className="relative h-16 w-48">
            <Image
              src="/placeholder.svg?height=160&width=480"
        ...
        <div className="flex items-center justify-between p-3 border-t border-[#a02020]">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="relative" ref={searchRef}>
              {showSearchInput && (
                <div
                  className={`fixed top-[80px] left-1/2 transform w-[100%] z-50 transition-all duration-300 ${
      ...
      {isSearchOpen && searchFocused && !isAllergenFilterOpen && (
        <div className="fixed top-[130px] inset-x-0 flex justify-center z-50">
          <div
            className={`${searchResultsStyles} transition-all duration-300 ${
              showSearchResults ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"
            }`}
          >
            {searchResultsHook.length > 0 ? (
              searchResultsHook.map((item, index) => (
                <div
                  ...
                  className={`p-3 hover:bg-gray-50 cursor-pointer border-b border-gray-100 last:border-b-0 transition-all duration-300 opacity-0 -translate-y-5`}
                  style={{
                    transitionDelay: `${index * 50}ms`,
                    opacity: showSearchResults ? 1 : 0,
                    transform: showSearchResults ? "translateY(0)" : "translateY(-5px)",
                  }}
